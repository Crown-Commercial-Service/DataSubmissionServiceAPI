...
RSpec.describe Workday::SubmitInvoiceAdjustment do
  ...
  let(:request) { Workday::SubmitInvoiceAdjustment.new(submission) }
  ...
  describe '#content' do
    ...
    describe 'when the invoice total is over £5000' do
      let(:submission) do
        FactoryBot.create(:submission_with_negative_management_charge_over_5k,
                          submitted_by: user,
                          task: task)
      end
