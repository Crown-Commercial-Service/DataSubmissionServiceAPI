...
RSpec.describe '/v1' do
  ...
  describe 'POST /v1/tasks/:task_id/no_business' do
    ...
    context 'if correcting an existing completed submission' do
      ...
      it 'updates the old submission to replaced' do
        post "/v1/tasks/#{task.id}/no_business", headers: json_headers.merge('X-Auth-Id' => JWT.encode(user.auth_id, "test")), params: body
        submission.reload
  ...
  describe 'PATCH /v1/tasks/:task_id' do
    it "updates a task's attributes" do
      ...
      patch "/v1/tasks/#{task.id}", params: params.to_json, headers: json_headers.merge('X-Auth-Id' => JWT.encode(user.auth_id, "test"))
  ...
  describe 'PATCH /v1/tasks/:task_id/cancel_correction' do
    it 'deletes incomplete correction submissions and updates the task status' do
      ...
      patch "/v1/tasks/#{task.id}/cancel_correction", headers: json_headers.merge('X-Auth-Id' => JWT.encode(user.auth_id, "test"))
