...
RSpec.describe 'V2::Agreements', type: :request do
  let(:valid_api_key) { create(:api_key) }
  let(:invalid_api_key) { 'invalid_key' }

  describe 'GET /v2/agreements' do
    context 'with valid API key' do
      before do
        create_list(:agreement, 3)
        get v2_agreements_path, headers: { 'API-Key' => valid_api_key.key }
      end
    ...
    context 'with invalid API key' do
      before { get v2_agreements_path, headers: { 'API-Key' => invalid_api_key } }
