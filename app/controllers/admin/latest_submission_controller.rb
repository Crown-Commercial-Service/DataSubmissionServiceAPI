class Admin::LatestSubmissionController < AdminController
  ...
  def task
    @task ||= Task.includes(:framework, :supplier, :latest_submission).find(params[:task_id])
  end

  def attachment
    @attachment ||= begin
      attachment = task.latest_submission.files.first.file
